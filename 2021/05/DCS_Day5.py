...
class Line:
    ...
    def handle_diagonal(self):
        ...
        self.line_length = self.y2 - self.y1
        if self.x2 > self.x1:
            self.x_direction = '+'
        else:
            self.x_direction = '-'
...
data_input = fileimport('day5_input.txt')
...
for i in range(0,len(input_list)):
    ...
    if line_list[i].direction == 'diagonal':
        ...
        #print(str(line_list[i].x1) + ',' + str(line_list[i].y1))
        #print(line_list[i].slope)
...
#diagram_5a = build_diagram(diagram_length,diagram_width)

#diagram_5a[1][3] = 1

for item in line_list:
    #coordinate_syntax = 'x' + str(i) + 'y' + str(j)]
...
for item in line_list:
    if item.direction == 'diagonal':
        if item.slope == 1 and item.x_direction == '+':
            for i in range(0,abs(item.line_length) + 1):
